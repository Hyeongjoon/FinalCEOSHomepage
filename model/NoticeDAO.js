...
exports.deletion = function(nid , callback){
	var sqlQuery = 'DELETE from notice WHERE nid = ' + mysql.escape(nid); 
	base.deletion(sqlQuery , callback);
}

exports.reviseBoard = function(noticeInfo , callback){
	var sqlQuery = 'UPDATE notice SET title = ' + mysql.escape(noticeInfo.title) + ', content = ' + mysql.escape(noticeInfo.content) + ' WHERE nid = ' + mysql.escape(noticeInfo.nid);
	base.update(sqlQuery , callback);
}
