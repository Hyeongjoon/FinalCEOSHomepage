...
router.get('/anony/click' , function(req , res){
	...
	async.waterfall([function(callback){
		...
		res.render('board_anony_det' , {targetBoardReply : targetBoardReply , targetBoardInfo : targetBoardInfo[0] , login_ck : req.session.passport}); //여기부터 하고가면될듯
		}
	});
});

router.get('/anony/make' , function(req , res){
	res.render('board_anony_make' , {login_ck : req.session.passport});
});

router.get('/notice/make' , function(req , res){
	if(req.session.passport.user.uid==clubMan){
		res.render('board_notice_make' , { login_ck : req.session.passport});
	} else{
...
router.post('/notice/delete' , function(req , res){
	var targetNid = req.body.bid; // req.body.bid;
	async.waterfall([function(callback){
		noticeDAO.deletion(targetNid , callback);
	}] , function(err ,result){
		if(err){
...
router.get('/anony/search', function(req, res){
	...
	async.waterfall([function(callback){
		...
		res.render('board_anony' , {board : boardInfo , totalNum : totalNum , search : search , whSearch : whSearch, pageNum : pageNum , login_ck : req.session.passport}); //search랑 whSearch 넘기는 이유는 Paging 버튼 눌러도 클라에서 바로바로 요청할수있게 할라고..
		}
	});
});
...
router.post('/anony/revise' , function(req , res){
	async.waterfall([function(callback){
		anonyDAO.findTagetAnonyByAnid(req.body.bid , callback);
	}] , function(err , result){
		if(err){
			res.send('내부서버오류');
		}else{
		res.render('board_anony_revise' , {login_ck : req.session.passport , targetBoardInfo : result[0]});
		}
	});
});

router.post('/anony/revision' , function(req , res){
	var inform = {
...
router.post('/anony/delete' , function(req , res){
	var targetAnid = req.body.bid; // req.body.bid;
	async.waterfall([function(callback){
...
router.post('/anony/deleteReply' , function(req , res){
	var anrid =  req.body.rid;//req.body.rid
	async.waterfall([function(callback){
...
router.get('/free/click' , function(req,res){ // 보드에서 클릭했을때 함수
	...
	async.waterfall([function(callback){
		...
		if(err){
			...
			//각 게시물에서 리플갯수 더하는거
		res.render('board_free_det' , { targetBoardReply : targetBoardReply , targetBoardInfo : targetBoardInfo[0] , login_ck : req.session.passport}); //여기부터 하고가면될듯
		}
	});
});
...
router.get('/free/search', function(req, res){
	...
	async.waterfall([function(callback){
		...
		if(err){
			res.render('board_free_search' , {board : undefined , totalNum : 'noBoard', pageNum : pageNum , login_ck : req.session.passport});
		} else{
			for(var i = 0 ; i < boardInfo.length ; i++){
				boardInfo[i].reply_num = 0;
				for(var j = 0; j < result.length ; j++){
		...
		res.render('board_free_search' , {board : boardInfo , totalNum : totalNum , search : search , whSearch : whSearch, pageNum : pageNum , login_ck : req.session.passport}); //search랑 whSearch 넘기는 이유는 Paging 버튼 눌러도 클라에서 바로바로 요청할수있게 할라고..
		}
	});
});
...
router.get('/free/myboard' , function(req , res){
	...
	async.waterfall([function(callback){
		...
		if(err){
			res.render('board_free_my' , {board : undefined , totalNum : 'noBoard', pageNum : pageNum , login_ck : req.session.passport});
		} else{
			for(var i = 0 ; i < boardInfo.length ; i++){
				boardInfo[i].reply_num = 0;
				boardInfo[i].writerName = req.session.passport.user.name;
		...
		res.render('board_free_my' , {board : boardInfo , totalNum : totalNum, pageNum : pageNum , login_ck : req.session.passport});
		}
	});
});
...
router.get('/free/make', function(req , res){
	res.render('board_free_make' , {login_ck : req.session.passport});
});

router.post('/free/revise' , function(req , res){
	async.waterfall([function(callback){
		boardDAO.findTagetBoardByBid(req.body.bid , callback);
	}] , function(err , result){
		if(err){
			res.send('내부서버오류');
		}else{
		res.render('board_free_revise' , {login_ck : req.session.passport , targetBoardInfo : result[0]});
		}
	});
});

router.post('/free/revision' , function(req , res){
	var inform = {
			title : req.body.title,//req.body.title;
			content : req.body.content, //req.body.content;
...
router.post('/free/deleteReply' , function(req , res){
...
router.post('/notice/revise' , function(req , res){
	async.waterfall([function(callback){
		noticeDAO.findTagetBoardByBid(req.body.bid , callback);
	}] , function(err , result){
		if(err){
			res.send('내부서버오류');
		}else{
		res.render('board_notice_revise' , {login_ck : req.session.passport , targetBoardInfo : result[0]});
		}
	});
});

router.post('/notice/revision' , function(req , res){
	var inform = {
			title : req.body.title,//req.body.title;
			content : req.body.content, //req.body.content;
			nid : req.body.bid //req.body.bid
	}
	async.waterfall([function(callback){
		noticeDAO.reviseBoard(inform , callback);
	}] , function(err , result){
		if(err){
			res.send('board 수정 안됨 내부오류');
		} else{
			res.redirect('/board/notice/click?nid='+req.body.bid);
		}
	});
});
