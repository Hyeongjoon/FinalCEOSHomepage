...
router.get('/qna', function(req, res){
	...
	async.waterfall([function(callback){
		...
		res.render('board_qna' , {board : boardInfo , totalNum : totalNum , pageNum : pageNum , login_ck : req.session.passport});
		}
	});
});
...
router.get('/qna/click' , function(req , res){ // qna 클릭했을때
	...
	async.waterfall([function(callback){
		...
		if(err){
			res.render('board_qna_det' , {board : undefined , totalNum : 'noBoard' , login_ck : req.session.passport});
		} else{
			for(var i = 0 ; i < boardInfo.length ; i++){
				boardInfo[i].reply_num = 0;
				for(var j = 0; j < result.length ; j++){
		...
		res.render('board_qna_det' , {targetBoardReply : targetBoardReply , targetBoardInfo : targetBoardInfo[0] , login_ck : req.session.passport}); //여기부터 하고가면될듯
		}
	});
});
...
router.get('/qna/search' , function(req , res){
	...
	async.waterfall([function(callback){
		...
		if(err){
			res.render('board_qna_search' , {board : undefined , totalNum : 'noBoard', pageNum : pageNum , login_ck : req.session.passport});
		} else{
			for(var i = 0 ; i < boardInfo.length ; i++){
				boardInfo[i].reply_num = 0;
				for(var j = 0; j < result.length ; j++){
		...
		res.render('board_qna_search' , {board : boardInfo , totalNum : totalNum , search : search , whSearch : whSearch, pageNum : pageNum , login_ck : req.session.passport}); //search랑 whSearch 넘기는 이유는 Paging 버튼 눌러도 클라에서 바로바로 요청할수있게 할라고..
		}
	});
});

router.get('/qna/make', function(req , res){
	res.render('board_qna_make' , {login_ck : req.session.passport});
});
...
router.post('/qna/revise' , function(req , res){
	async.waterfall([function(callback){
		...
		res.render('board_qna_revise' , {login_ck : req.session.passport , targetBoardInfo : result[0]});
		}
	});
});
...
router.get('/notice' , function(req, res){
	...
	async.waterfall([function(callback){
		...
		if(err){
			res.render('board_notice' , {board : undefined , totalNum : 'noBoard' , login_ck : req.session.passport});
		} else{
			for(var i = 0 ; i < boardInfo.length ; i++){
				var d = new Date(boardInfo[i].written_time);
				var temp;
		...
		res.render('board_notice' , {board : boardInfo , totalNum : totalNum , pageNum : pageNum , login_ck : req.session.passport});
		}
	});
});
...
router.get('/notice/click' , function(req,res){ // 보드에서 클릭했을때 함수
	...
	async.waterfall([function(callback){
		...
		if(err){
			...
			targetBoardInfo[0].written_time = d.getFullYear()+"-"+temp+"-"+d.getDate();
			console.log(targetBoardInfo[0]);
			res.render('board_notice_det' , { targetBoardInfo : targetBoardInfo[0] , login_ck : req.session.passport}); //여기부터 하고가면될듯
		}
	});
});

router.get('/faq' , function(req , res){
	res.render('board_faq' , {login_ck : req.session.passport});
});
...
router.get('/anony', function(req, res ){
	...
	async.waterfall([function(callback){
		...
		if(err){
			res.render('board_anony' , {board : undefined , totalNum : 'noBoard' , login_ck : req.session.passport});
		} else{
			for(var i = 0 ; i < boardInfo.length ; i++){
				boardInfo[i].reply_num = 0;
				for(var j = 0; j < result.length ; j++){
		...
		res.render('board_anony' , {board : boardInfo , totalNum : totalNum , pageNum : pageNum , login_ck : req.session.passport});
		}
	});
});
...
router.get('/free', function(req, res){
	...
	async.waterfall([function(callback){
		...
		if(err){
			res.render('board_free' , {board : undefined , totalNum : 'noBoard' , login_ck : req.session.passport});
		} else{
			for(var i = 0 ; i < boardInfo.length ; i++){
				boardInfo[i].reply_num = 0;
				for(var j = 0; j < result.length ; j++){
		...
		res.render('board_free' , {board : boardInfo , totalNum : totalNum , pageNum : pageNum , login_ck : req.session.passport});
		}
	});
});
